...
module.get_urls = function(file, url, is_css, iri)
	if cur_stat_code == 200 then
		...
		for _, v in pairs(res["files"]) do
			queue_request({url="https://clara.io/resources/" .. v["hash"] .. v["suffix"] .. "?filename=" .. minimal_escape(v["name"])}, "resources_must_be_found", true)
			queue_request({url="https://clara.io/resources/" .. v["hash"] .. "lzma1" .. "?filename=" .. minimal_escape(v["name"])}, "resources_maybe_found", true) -- I cannot be bothered to write a parser for the thing that determines whether these are present, hence why 404 is allowd.
		end
